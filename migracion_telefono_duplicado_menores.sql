-- ============================================================
-- El registro rechazaba un jugador nuevo si su teléfono ya estaba
-- usado por otro jugador ("Ese número de WhatsApp ya está registrado
-- con otro jugador"). Eso rompe con menores de edad: varios hermanos
-- menores comparten el mismo número (el de un papá/mamá), y el
-- organizador necesita poder registrarlos a todos igual.
--
-- Ahora esa validación de teléfono repetido SOLO aplica a jugadores
-- adultos. Si el jugador que se está registrando es menor de edad
-- (según su fecha de nacimiento), se deja pasar aunque el teléfono ya
-- esté en otro jugador.
--
-- registrar_equipo se reescribe completa (igual que en cada migración
-- anterior) — el resto de la función queda EXACTAMENTE igual que en
-- migracion_duracion_link_registro.sql, solo cambia el chequeo de
-- teléfono duplicado.
--
...
begin
  ...
  -- Link vencido (según registro_token_horas del equipo — 1, 2 o 3 días
  -- elegidos al compartirlo; si no hay nada guardado, se asume 24h)
  if v_equipo.registro_token_generado_en is not null
     and v_equipo.registro_token_generado_en < now() - (coalesce(v_equipo.registro_token_horas, 24) || ' hours')::interval then
  ...
  if not found then
    ...
    -- WhatsApp único (últimos 10 dígitos) — solo si vino teléfono Y el
    -- jugador es adulto. Los menores de edad suelen compartir el número
    -- de un adulto responsable con sus hermanos, así que para ellos no
    -- se exige que el teléfono sea único.
    if not v_es_menor and nullif(trim(coalesce(p_telefono, '')), '') is not null then
      begin
