-- ============================================================
-- ARREGLO: las sanciones (expulsión/roja) se mezclaban entre torneos.
--
-- Las sanciones automáticas por tarjeta roja SÍ quedan guardadas con el
-- tournament_id correcto (ver PlanillaRapida.jsx / PlanillaPartido.jsx), y
-- las planillas y el panel del torneo SÍ filtran bien (tournament_id = este
-- torneo O tournament_id IS NULL para las sanciones globales que pone el
-- admin principal a mano).
--
-- El problema estaba en jugador_sancion_activa(p_player_id) — la función
-- que usa el LINK DE REGISTRO para avisar "jugador sancionado, no puede
-- inscribirse". Esa función NO recibía el torneo: miraba si el jugador
-- tenía CUALQUIER sanción activa, sin importar de qué torneo era. Por eso
-- una roja en "Los Amigos" bloqueaba/mostraba sancionado también en
-- "Santa Rita".
--
-- Se le agrega el parámetro p_tournament_id (con default null, para no
-- romper si algo la llama sin ese dato) y se filtra igual que en el resto
-- de la app: tournament_id = este torneo, o tournament_id IS NULL (sanción
-- global puesta a mano por el admin principal — esa sí debe aplicar en
-- todos lados, es la única excepción a propósito).
--
-- Cómo ejecutar: Supabase → SQL Editor → pegar todo → RUN.
-- Es seguro ejecutarlo más de una vez.
-- ============================================================

create or replace function public.jugador_sancion_activa(p_player_id uuid, p_tournament_id uuid default null)
returns jsonb
language plpgsql
security definer
...
revoke all on function public.jugador_sancion_activa(uuid, uuid) from public;
grant execute on function public.jugador_sancion_activa(uuid, uuid) to anon, authenticated;

-- La firma vieja (1 solo parámetro) queda huérfana una vez registrar_equipo
-- pasa a llamar siempre la de 2 parámetros — se borra para que no queden
-- dos versiones compitiendo.
drop function if exists public.jugador_sancion_activa(uuid);

-- registrar_equipo: se reescribe completa (igual que en cada migración
-- anterior) solo para pasarle el torneo a jugador_sancion_activa. El resto
-- de la función queda EXACTAMENTE igual que en migracion_autorizacion_menor_edad.sql.
...
begin
  ...
  begin
    v_sanc := public.jugador_sancion_activa(v_player.id, p_tournament_id);
