  ...
  useEffect(() => {
    async function load() {
      ...
      ;(mData || []).filter(m => !m.fase || m.fase === 'grupo').forEach(m => {
        ...
        if (h) {
          h.pj++; h.gf += m.home_score||0; h.gc += m.away_score||0
          if (m.home_score > m.away_score) { h.pg++; h.pts += 3 }
          else if (m.home_score === m.away_score) { h.pe++; h.pts++ }
          else h.pp++
        }
        if (a) {
          a.pj++; a.gf += m.away_score||0; a.gc += m.home_score||0
          if (m.away_score > m.home_score) { a.pg++; a.pts += 3 }
          else if (m.away_score === m.home_score) { a.pe++; a.pts++ }
          else a.pp++
        }
      })
      ...
      const form = teamMatches.map(m => {
        ...
        const th = isHome ? m.away_score : m.home_score
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.88)', zIndex:500,
      ...
      <div style={{ background:S.surface, borderRadius:'20px 20px 0 0', width:'100%',
        ...
        <div style={{ padding:'16px 20px 12px', borderBottom:`0.5px solid ${S.border}`,
          ...
          <div style={{ width:'44px', height:'44px', borderRadius:'10px', background:S.card2,
            overflow:'hidden', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
            {teamLogo
              ? <img src={teamLogo} style={{ width:'100%', height:'100%', objectFit:'contain' }}/>
              : <span style={{ fontSize:'1rem', fontWeight:'800', color:'#fff' }}>{(teamName||'?').substring(0,2).toUpperCase()}</span>}
          </div>
          <div style={{ flex:1 }}>
          ...
          <div style={{ padding:'16px 20px 32px', display:'flex', flexDirection:'column', gap:'12px' }}>
            ...
            <div style={{ background:S.card, borderRadius:'14px', padding:'14px 16px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'8px' }}>
                {[['PJ', data.myStats.pj, S.text2],
                  ['PG', data.myStats.pg, S.win],
                  ['PE', data.myStats.pe, S.gold],
            ...
            {data.form.length > 0 && (
              <div style={{ background:S.card, borderRadius:'14px', padding:'14px 16px' }}>
                ...
                <div style={{ display:'flex', gap:'8px' }}>
                  {data.form.map((r,i) => (
                    <div key={i} style={{ width:'38px', height:'38px', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center',
                      background:FB[r], border:`1px solid ${FC[r]}`, fontWeight:'900', fontSize:'.85rem', color:FC[r] }}>{r}</div>
                  ))}
                  {data.form.length < 5 && [...Array(5-data.form.length)].map((_,i) => (
                    <div key={'e'+i} style={{ width:'38px', height:'38px', borderRadius:'10px', background:'rgba(30,45,61,.4)', border:`1px dashed ${S.border}` }}/>
                  ))}
                </div>
                <div style={{ fontSize:'.65rem', color:S.muted, marginTop:'6px' }}>
                  {['W=Victoria','D=Empate','L=Derrota'].join(' · ')}
                </div>
              </div>
            )}
            ...
            <div style={{ background:S.card, borderRadius:'14px', padding:'14px 16px' }}>
              ...
              {data.tablaRows.map(r => (
                <div key={r.team.id} style={{ display:'grid', gridTemplateColumns:'28px 1fr 30px 30px 30px 36px', alignItems:'center', padding:'6px 4px',
                  borderRadius:'8px', background: r.isMe ? S.cyanDim : 'transparent',
                  border: r.isMe ? `1px solid ${S.cyan}` : '1px solid transparent', marginBottom:'2px' }}>
            ...
            {data.scorers.length > 0 && (
              <div style={{ background:S.card, borderRadius:'14px', padding:'14px 16px' }}>
                ...
                {data.scorers.map((s,i) => (
                  <div key={s.player_id} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 0',
                    borderBottom: i < data.scorers.length-1 ? `1px solid ${S.border}` : 'none' }}>
                    <div style={{ width:'30px', height:'30px', borderRadius:'50%', background:S.card2,
                      overflow:'hidden', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
...
export default function PlayerApuestasPage() {
  ...
  async function guardarPrediccion() {
    ...
    const data = {
      ...
      goleador_id: form.goleadorId || null,
    }
    if (pred) {
  ...
  const gruposPendientes = groupByTournament(
              ...
              <div style={{ padding:'12px 20px', borderBottom:`0.5px solid ${S.border}`,
                ...
                {['Resultado','Marcador','Goleador','Confirmar'].map((s,i) => (
                  <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:'4px' }}>
                    <div style={{ width:'24px', height:'24px', borderRadius:'50%', fontSize:'.7rem', fontWeight:'700',
              ...
              {step === 1 && (
                <div>
                  ...
                  <button disabled={!form.ganador} onClick={() => setStep(2)}
                    style={{ width:'100%', marginTop:'12px', padding:'12px', background: form.ganador?S.cyan:S.border,
                      border:'none', borderRadius:'10px', cursor: form.ganador?'pointer':'not-allowed',
                      color: form.ganador?'#000':S.muted, fontWeight:'700', fontSize:'.875rem' }}>
                    CONTINUAR →
                  </button>
                </div>
              )}
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'32px', marginBottom:'20px' }}>
                    {[0,1].map(team => (
                      <div key={team} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'10px' }}>
                        <div style={{ fontSize:'.75rem', color:S.muted, textAlign:'center' }}>
                          {team===0 ? modal.home?.name : modal.away?.name}
                        </div>
                        <div style={{ display:'flex', alignItems:'center', gap:'14px' }}>
                          ...
                          <div style={{ fontSize:'2.5rem', fontWeight:'800', color:S.cyan, minWidth:'36px', textAlign:'center' }}>
                            {team===0 ? form.golesHome : form.golesAway}
                          </div>
                          <button onClick={() => setForm(f => ({ ...f, [team===0?'golesHome':'golesAway']: (team===0?f.golesHome:f.golesAway)+1 }))}
                            style={{ width:'40px', height:'40px', borderRadius:'50%', border:`1px solid ${S.border}`, background:S.card, color:S.text, cursor:'pointer', fontSize:'1.2rem', fontWeight:'700' }}>+</button>
                        </div>
                      </div>
                    ))}
                ...
                <div>
                  ...
                  <div onClick={() => setForm(f => ({ ...f, goleadorId: null }))}
                    style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', borderRadius:'10px', cursor:'pointer', marginBottom:'8px',
                      ...
                      background: form.goleadorId===null ? 'rgba(154,160,166,.08)' : S.card }}>
                    <div style={{ width:'32px', height:'32px', borderRadius:'50%', background:S.card2, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    ...
                    {jugsModal.length === 0 ? (
                      ...
                      <div key={j.id} onClick={() => setForm(f => ({ ...f, goleadorId: j.id }))}
                        style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', borderRadius:'10px', cursor:'pointer',
                          border:`1px solid ${form.goleadorId===j.id ? S.cyan : S.border}`,
                          background: form.goleadorId===j.id ? S.cyanDim : S.card, transition:'all .15s' }}>
                ...
                <div>
                  ...
                  <div style={{ display:'flex', gap:'8px', marginTop:'20px' }}>
                    ...
                    <button onClick={guardarPrediccion} disabled={guardando}
                      style={{ flex:2, padding:'11px', background:S.win, border:'none', borderRadius:'10px', cursor: guardando?'not-allowed':'pointer', color:'#fff', fontWeight:'700', fontSize:'.875rem', opacity: guardando?.7:1 }}>
              ...
              {step === 5 && (
                <div style={{ textAlign:'center', padding:'20px 0' }}>
                  ...
                  <div style={{ fontSize:'.85rem', color:S.muted }}>
                    {miscPreds[modal?.id]?.ganador === 'home' ? modal?.home?.name+' gana'
                      : miscPreds[modal?.id]?.ganador === 'away' ? modal?.away?.name+' gana'
                      : 'Empate'}
                    {' · '}{miscPreds[modal?.id]?.goles_home} – {miscPreds[modal?.id]?.goles_away}
                  </div>
                  <button onClick={() => setModal(null)} style={{ marginTop:'20px', padding:'10px 28px', background:S.card, border:`1px solid ${S.border}`, borderRadius:'10px', cursor:'pointer', color:S.text, fontWeight:'600', fontSize:'.875rem' }}>
                    Cerrar
                  </button>
                </div>
              )}
      ...
      <div style={{ maxWidth:'600px', margin:'0 auto', padding:'0 16px' }}>
        ...
        <div style={{ display:'flex', gap:'4px', padding:'12px 0', position:'sticky', top:0, background:S.navy, zIndex:10 }}>
          {[
            ...
            { id:'mias',       label:'Mis predicciones' },
            { id:'ranking',    label:'Ranking' },
          ].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ flex:1, padding:'9px 4px', borderRadius:'10px', border:'none', cursor:'pointer', fontSize:'.78rem', fontWeight:'700', transition:'all .15s',
                background: tab===t.id ? S.cyan : S.card,
                color:      tab===t.id ? '#000'  : S.muted }}>
              {t.label}
            </button>
          ))}
        </div>
              ...
              <div style={{ display:'flex', gap:'8px', overflowX:'auto', paddingBottom:'8px', marginBottom:'12px',
                ...
                {[{ id:null, name:'Todos' }, ...torneosUnicos].map(t => (
                  <button key={t?.id||'all'} onClick={() => setTorneoFiltro(t?.id||null)}
                    style={{ flexShrink:0, padding:'6px 14px', borderRadius:'20px', border:'none', cursor:'pointer',
                      fontWeight:'600', fontSize:'.75rem', whiteSpace:'nowrap',
                      background: torneoFiltro===(t?.id||null) ? S.cyan : S.card,
                      color:      torneoFiltro===(t?.id||null) ? '#000'  : S.muted }}>
                    {t?.name||'Todos'}
                  </button>
                ))}
              </div>
              ...
              return (
                <div key={tid} style={{ marginBottom:'16px', background:S.card, borderRadius:'16px', overflow:'hidden', border:`0.5px solid ${S.border}` }}>
                  <div onClick={() => toggleCollapse(tid)}
                    ...
                    <div style={{ width:'32px', height:'32px', borderRadius:'8px', background:'rgba(0,221,208,.1)',
                      display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      <Trophy size={16} color={S.cyan}/>
                    </div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontWeight:'700', fontSize:'.9rem', color:S.text, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                        {grupo.torneo?.name || 'Torneo'}
                      </div>
        ...
        {tab === 'mias' && (
          <div>
            {Object.keys(miscPreds).length === 0 ? (
              <div style={{ textAlign:'center', padding:'60px 20px', color:S.muted }}>
                <div style={{ fontSize:'2rem', marginBottom:'12px' }}>🎯</div>
                <div>Aún no tienes predicciones</div>
              </div>
                  ...
                  return (
                    <div key={p.id} style={{ background:S.card, borderRadius:'14px', padding:'14px 16px', marginBottom:'10px', border:`0.5px solid ${pred.resuelta ? (acerto?S.win:S.border) : S.border}` }}>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'8px' }}>
                        ...
                        {pred.resuelta
                          ? <span style={{ fontSize:'.75rem', fontWeight:'800', color: acerto?S.gold:S.muted, background: acerto?S.goldDim:'rgba(154,160,166,.1)', borderRadius:'10px', padding:'2px 10px' }}>
                              {acerto ? `+${pred.puntos_ganados} pts ✓` : '0 pts'}
                            </span>
            ...
            {ranking.length === 0 ? (
              ...
              return (
                <div key={r.id} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px 16px',
                  background: esYo ? S.cyanDim : S.card, borderRadius:'12px', marginBottom:'8px',
                  border: `0.5px solid ${esYo ? S.cyan : S.border}` }}>
                  <div style={{ width:'28px', fontWeight:'900', fontSize:'.9rem', textAlign:'center', flexShrink:0,
                    color: i===0?S.gold : i===1?'#9aa0a6' : i===2?'#cd7f32' : S.muted }}>
                    {i===0 ? '🥇' : i===1 ? '🥈' : i===2 ? '🥉' : `#${i+1}`}
                  </div>
