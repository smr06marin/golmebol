...
const EMPTY = { name:'', fecha_nacimiento:'', numero_cedula:'', genero:'', telefono:'', acudiente_nombre:'', acudiente_telefono:'' }
...
export default function EscuelaJugadoresPage() {
  ...
  function abrirEditar(j) {
    setForm({
      ...
      genero: j.genero || '', telefono: j.telefono || '',
  ...
  async function handleGuardar() {
    ...
    const { data: nuevo, error: errIns } = await supabase.from('players')
      .insert({ ...form, activo_membresia: true, fecha_registro: new Date().toISOString() })
      .select().single()
    ...
    <div style={{ minHeight:'100vh', background:S.navy, fontFamily:'system-ui,sans-serif', color:S.text, paddingBottom:'40px' }}>
      ...
      <div style={{ maxWidth:'640px', margin:'0 auto', padding:'18px 16px' }}>
        ...
        {filtrados.length === 0 ? (
          ...
          return (
            <div key={j.id} style={{ background:S.card, border:`1px solid ${S.border}`, borderRadius:'14px', padding:'12px 16px', marginBottom:'8px', display:'flex', alignItems:'center', gap:'12px' }}>
              ...
              {esCoordinador && (
                <button onClick={() => abrirEditar(j)} style={{ background:'none', border:`1px solid ${S.border}`, borderRadius:'8px', padding:'6px 10px', cursor:'pointer', color:S.muted, fontSize:'.78rem' }}>✏️</button>
              )}
            </div>
          )
        })}
