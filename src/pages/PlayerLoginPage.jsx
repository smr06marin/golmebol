...
export default function PlayerLoginPage() {
  ...
  async function handleBuscarCedula(e) {
    ...
    const { data: p } = await supabase
      .from('players')
      .select('id, name, activo_membresia, fecha_vencimiento, user_id, primer_ingreso, rol, es_arbitro')
  ...
  async function handleLogin(e) {
    ...
    const { data: pActual } = await supabase.from('players').select('activo_membresia, fecha_vencimiento, primer_ingreso, rol, es_arbitro').eq('id', player.id).single()
  ...
  if (splash) return (
    <SponsorSplash
      ...
      onDone={() => navigate(player?.rol === 'arbitro' && !player?.es_arbitro_y_jugador ? '/arbitro' : '/jugador')}
    />
  )
