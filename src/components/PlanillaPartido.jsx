...
// Modal MVP — obligatorio al cerrar
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '420px', boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}>
        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
          ...
          <div style={{ fontSize: '.72rem', color: '#d93025', marginTop: '6px', fontWeight: '600' }}>* Obligatorio para cerrar la planilla</div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ minHeight: '44px', background: jugador ? '#e6f4ea' : numero && equipo ? '#fce8e6' : '#f8f9fa', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px', border: `1px solid ${jugador ? '#ceead6' : numero && equipo ? '#fad2cf' : '#e8eaed'}` }}>
            {jugador ? (
              ...
              <><span style={{ fontSize: '1.4rem' }}>❌</span><div style={{ fontSize: '.82rem', color: '#d93025' }}>No se encontró camiseta #{numero} en este equipo</div></>
            ) : (
          ...
          <button onClick={onSaltear}
            style={{ padding: '10px', background: 'none', border: '1px solid #dadce0', borderRadius: '10px', cursor: 'pointer', color: '#9aa0a6', fontSize: '.78rem' }}>
...
// Modal Partido por W / Desierto
function ModalEspecial({ tipo, partido, onConfirmar, onCancelar }) {
  const esW       = tipo === 'w'
  const titulo    = esW ? '🏆 Partido por W' : '❌ Partido Desierto'
  const descripcion = esW
    ? 'Un equipo no se presentó. El equipo que SÍ asistió gana 3 a 0.'
    : 'Ninguno de los dos equipos se presentó. El partido queda 0 a 0 anulado.'
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '400px', boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}>
        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
          ...
          <div style={{ fontWeight: '700', color: '#202124', fontSize: '1.1rem' }}>{titulo}</div>
          <div style={{ fontSize: '.82rem', color: '#5f6368', marginTop: '8px', lineHeight: 1.5 }}>{descripcion}</div>
        </div>
        ...
        <div style={{ background: '#fff8e1', border: '1px solid #ffe082', borderRadius: '10px', padding: '12px 14px', marginBottom: '16px' }}>
          ...
          <div style={{ fontSize: '.75rem', color: '#795548' }}>
            Este partido <b>no contará</b> para las predicciones. Se mostrará el mensaje:<br/>
            <i>"{esW ? 'Partido no cuenta para predicciones (W)' : 'Partido no cuenta para predicciones (Desierto)'}"</i>
          </div>
        </div>
        ...
        {esW && (
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setEquipoGana('local')}
                style={{ flex: 1, padding: '10px', border: `2px solid ${equipoGana === 'local' ? '#1e8e3e' : '#dadce0'}`, borderRadius: '8px', cursor: 'pointer', background: equipoGana === 'local' ? '#e6f4ea' : '#fff', color: equipoGana === 'local' ? '#1e8e3e' : '#5f6368', fontWeight: equipoGana === 'local' ? '700' : '400', fontSize: '.82rem' }}>
                {partido.home?.name}
              </button>
              <button onClick={() => setEquipoGana('visitante')}
                style={{ flex: 1, padding: '10px', border: `2px solid ${equipoGana === 'visitante' ? '#1e8e3e' : '#dadce0'}`, borderRadius: '8px', cursor: 'pointer', background: equipoGana === 'visitante' ? '#e6f4ea' : '#fff', color: equipoGana === 'visitante' ? '#1e8e3e' : '#5f6368', fontWeight: equipoGana === 'visitante' ? '700' : '400', fontSize: '.82rem' }}>
                {partido.away?.name}
              </button>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button onClick={onCancelar}
            style={{ padding: '11px 16px', background: '#fff', border: '1px solid #dadce0', borderRadius: '10px', cursor: 'pointer', color: '#5f6368', fontSize: '.875rem' }}>
            Cancelar
          </button>
        </div>
      </div>
...
export default function PlanillaPartido({ partido, onClose, onGuardarResultado }) {
  ...
  const [arbitrosTorneo,     setArbitrosTorneo]     = useState([]) // árbitros del torneo
  ...
  const [showEspecial,       setShowEspecial]       = useState(null) // 'w' | 'desierto' | null

  // Penales
  const [hubopenales,        setHuboPenales]        = useState(false)
  const [penalesGanador,     setPenalesGanador]     = useState('') // 'local' | 'visitante'
  ...
  useEffect(() => {
    ...
    const snap = {
      ...
      huboPenales: hubopenales, penalesGanador, penalesLocal, penalesVisitante,
    ...
    horaInicio1, horaFin1, horaInicio2, horaFin2,
    tiroInicial, colorLocal, colorVisitante, duracionMinutos, mvpId,
    hubopenales, penalesGanador, penalesLocal, penalesVisitante
  ])
  ...
  // Cargar árbitros del torneo desde localStorage
  useEffect(() => {
    try {
      const stored = localStorage.getItem(arbitrosKey)
      if (stored) setArbitrosTorneo(JSON.parse(stored))
    } catch(e) {}
  }, [])
  ...
  // Guardar árbitro en localStorage por torneo al escribirlo
  function guardarArbitroLocal(nombre) {
    ...
    try {
      ...
      if (!lista.includes(nombre.trim())) {
        const nueva = [...lista, nombre.trim()].slice(-20) // max 20
  ...
  async function fetchTodo() {
    ...
    if (!navigator.onLine) {
      try {
        ...
        if (snap) {
          ...
          setHoraInicio1(s.horaInicio1 || ''); setHoraFin1(s.horaFin1 || '')
          setHoraInicio2(s.horaInicio2 || ''); setHoraFin2(s.horaFin2 || '')
          ...
          if (s.huboPenales) setHuboPenales(s.huboPenales)
          if (s.penalesGanador) setPenalesGanador(s.penalesGanador)
          if (s.penalesLocal) setPenalesLocal(s.penalesLocal)
          if (s.penalesVisitante) setPenalesVisitante(s.penalesVisitante)
    ...
    if (yaJugado) {
      const applyTarjetas = (jugs, statsArr) => jugs.map(j => {
        ...
        if (!s) return j
        return { ...j, amarilla: s.yellow_cards > 0, azul: s.blue_cards > 0, roja: s.red_cards > 0, faltasPeriodo: Array(s.fouls || 0).fill(1) }
      })
    ...
    try {
      ...
      if (local) {
        ...
        if (snap.pendienteSync) {
          ...
          setAnotador(snap.anotador || ''); setCronometroNombre(snap.cronometroNombre || '')
          setObservaciones(snap.observaciones || '')
          setHoraInicio1(snap.horaInicio1 || ''); setHoraFin1(snap.horaFin1 || '')
          setHoraInicio2(snap.horaInicio2 || ''); setHoraFin2(snap.horaFin2 || '')
          setTiroInicial(snap.tiroInicial || null)
          setColorLocal(snap.colorLocal || '#1a3a8a'); setColorVisitante(snap.colorVisitante || '#d93025')
          if (snap.mvpId) setMvpId(snap.mvpId)
          if (snap.huboPenales) setHuboPenales(snap.huboPenales)
          if (snap.penalesGanador) setPenalesGanador(snap.penalesGanador)
          if (snap.penalesLocal) setPenalesLocal(snap.penalesLocal)
          if (snap.penalesVisitante) setPenalesVisitante(snap.penalesVisitante)
    ...
    try {
      localStorage.setItem(cacheJugsKey, JSON.stringify({ jugadoresLocal: jugsLocalBase, jugadoresVisitante: jugsVisBase, torneo: torneoData, duracion: getDefaultDuracion(torneoData?.modalidad), savedAt: new Date().toISOString() }))
    } catch(e) {}
    setLoading(false)
  }

  async function guardarEnDB(esEspecial = null) {
    setGuardandoDB(true)
    let golesLocalTotal, golesVisTotal, tipoPartido = null

    if (esEspecial) {
      tipoPartido = esEspecial.tipo // 'w' | 'desierto'
      if (esEspecial.tipo === 'w') {
        golesLocalTotal    = esEspecial.equipoGana === 'local' ? 3 : 0
        golesVisTotal      = esEspecial.equipoGana === 'visitante' ? 3 : 0
      } else {
        golesLocalTotal = 0; golesVisTotal = 0
      }
    } else {
    ...
    // Actualizar partido con tipo especial y penales si aplica
    const updatePartido = {
      home_score: golesLocalTotal,
      away_score: golesVisTotal,
      status: 'finished',
    }
    if (tipoPartido) updatePartido.tipo_resultado = tipoPartido
    if (hubopenales) {
      updatePartido.penales_local    = parseInt(penalesLocal)    || 0
      updatePartido.penales_visitante= parseInt(penalesVisitante)|| 0
      updatePartido.penales_ganador  = penalesGanador
    }
    ...
    const procesarStats = (jugadores, goles, team_id, esLocal) => {
      ...
      jugadores.forEach(j => {
        // Solo sumar partido jugado si tiene número de camiseta
    ...
    if (statsRows.length > 0) {
      await supabase.from('player_match_stats').upsert(statsRows, { onConflict: 'match_id,player_id' })
    }

    // Predicciones — no sumar puntos si es W o desierto
    if (!tipoPartido) {
      ...
      // Marcar predicciones como resueltas sin puntos
      await supabase.from('predicciones').update({ puntos_ganados: 0, resuelta: true, nota: `Partido no cuenta (${tipoPartido})` }).eq('match_id', partido.id).eq('resuelta', false)
    }
  ...
  async function handleConfirmarEspecial(info) {
    ...
    await guardarEnDB(info)
    // No mostrar MVP en partidos W o desierto
    onClose()
  }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1px' }}>
          ...
          {gol ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '1px', flex: 1 }}
              onDoubleClick={() => { if (esUl) eliminarGol(equipo, slotIdx) }} title={esUl ? 'Doble click para eliminar' : ''}>
          ...
          return (
            <tr key={idx} style={{ height: '17px', background: esMVP ? '#fff8e1' : 'transparent' }}>
              ...
              {[0,1,2,3,4].map(n => {
                ...
                return <td key={n} style={{ ...cell, background: tF ? '#e0e0e0' : 'transparent' }}>{tF && <span style={{ color: '#111', fontWeight: '700' }}>{n+1}</span>}</td>
              })}
  ...
  // Sugerencias árbitros filtradas
  ...
  return (
    <>
      ...
      {showEspecial && (
        <ModalEspecial
          tipo={showEspecial} partido={partido}
          onConfirmar={handleConfirmarEspecial}
          onCancelar={() => setShowEspecial(null)}
        />
      )}
        ...
        <div style={{ background: '#fff', borderRadius: '12px', width: '100%', maxWidth: '980px', marginBottom: '20px' }}>
          ...
          <div className="no-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid #e8eaed', flexWrap: 'wrap', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {/* Botones especiales */}
              <button onClick={() => setShowEspecial('w')}
                style={{ padding: '6px 12px', background: '#e8710a', border: 'none', borderRadius: '8px', cursor: 'pointer', color: '#fff', fontSize: '.78rem', fontWeight: '700' }}>
                🏆 Partido por W
              </button>
              <button onClick={() => setShowEspecial('desierto')}
                style={{ padding: '6px 12px', background: '#5f6368', border: 'none', borderRadius: '8px', cursor: 'pointer', color: '#fff', fontSize: '.78rem', fontWeight: '700' }}>
                ❌ Desierto
              </button>
            ...
            {hubopenales && (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <input type="number" min="0" value={penalesLocal} onChange={e => setPenalesLocal(e.target.value)}
                    style={{ width: '40px', textAlign: 'center', border: '1px solid #dadce0', borderRadius: '6px', padding: '3px 6px', fontSize: '.85rem', fontWeight: '700' }}/>
                </div>
                <span style={{ fontWeight: '700', color: '#5f6368' }}>—</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ fontSize: '.75rem', color: '#5f6368' }}>{partido.away?.name}:</span>
                  <input type="number" min="0" value={penalesVisitante} onChange={e => setPenalesVisitante(e.target.value)}
                    style={{ width: '40px', textAlign: 'center', border: '1px solid #dadce0', borderRadius: '6px', padding: '3px 6px', fontSize: '.85rem', fontWeight: '700' }}/>
                </div>
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                  <span style={{ fontSize: '.75rem', color: '#5f6368' }}>Ganador:</span>
                  <button onClick={() => setPenalesGanador('local')}
                    style={{ padding: '4px 10px', border: `2px solid ${penalesGanador === 'local' ? '#1e8e3e' : '#dadce0'}`, borderRadius: '6px', cursor: 'pointer', background: penalesGanador === 'local' ? '#e6f4ea' : '#fff', color: penalesGanador === 'local' ? '#1e8e3e' : '#5f6368', fontSize: '.75rem', fontWeight: penalesGanador === 'local' ? '700' : '400' }}>
                    {partido.home?.name}
                  </button>
                  <button onClick={() => setPenalesGanador('visitante')}
                    style={{ padding: '4px 10px', border: `2px solid ${penalesGanador === 'visitante' ? '#1e8e3e' : '#dadce0'}`, borderRadius: '6px', cursor: 'pointer', background: penalesGanador === 'visitante' ? '#e6f4ea' : '#fff', color: penalesGanador === 'visitante' ? '#1e8e3e' : '#5f6368', fontSize: '.75rem', fontWeight: penalesGanador === 'visitante' ? '700' : '400' }}>
                    {partido.away?.name}
                  </button>
                </div>
              </>
            )}
            <div style={{ marginLeft: 'auto', fontSize: '.7rem', color: '#9aa0a6' }}>
              💡 <b>Doble click</b> para borrar datos · Goles y faltas: doble click en el último para eliminar
            </div>
          </div>
          ...
          <div id="planilla-print" style={{ padding: '10px', fontFamily: 'Arial, sans-serif' }}>
              ...
              <tbody>
                ...
                <tr>
                  <td colSpan={3} style={cell}>
                    ...
                    {hubopenales && penalesGanador && (
                      <>&nbsp;<b>PENALES:</b> <span style={{ fontWeight:'700', color:'#1a73e8' }}>{partido.home?.name} {penalesLocal} — {penalesVisitante} {partido.away?.name} · Gana: {penalesGanador === 'local' ? partido.home?.name : partido.away?.name}</span></>
                    )}
                  </td>
                </tr>
              </tbody>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                <tr>
                  {/* Árbitro 1 con autocompletado */}
                  <td style={{ ...cellL, width: '30%', position: 'relative' }}>
                    <b>ÁRBITRO 1:</b>
                    <input value={arbitroInput1}
                      onChange={e => { setArbitroInput1(e.target.value); setArbitro1(e.target.value); setShowSuggest1(true) }}
                      onBlur={() => { setTimeout(() => setShowSuggest1(false), 150); guardarArbitroLocal(arbitroInput1) }}
                      onFocus={() => setShowSuggest1(true)}
                      placeholder="Nombre árbitro..."
                      style={{ ...inpL, width: '120px', display: 'inline', borderBottom: '1px solid #000', marginLeft: '3px' }}/>
                    {showSuggest1 && suggestFiltro1.length > 0 && (
                      <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 500, background: '#fff', border: '1px solid #dadce0', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,.15)', minWidth: '180px', maxHeight: '120px', overflowY: 'auto' }}>
                        {suggestFiltro1.map((a, i) => (
                          <div key={i} onClick={() => { setArbitroInput1(a); setArbitro1(a); setShowSuggest1(false) }}
                            style={{ padding: '5px 10px', cursor: 'pointer', fontSize: '9px', borderBottom: '1px solid #f1f3f4' }}
                            onMouseEnter={e => e.currentTarget.style.background = '#f8f9fa'}
                            onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
                            {a}
                          </div>
                  ...
                  {/* Árbitro 2 con autocompletado */}
                  <td style={{ ...cellL, width: '30%', position: 'relative' }}>
                    <b>ÁRBITRO 2:</b>
                    <input value={arbitroInput2}
                      onChange={e => { setArbitroInput2(e.target.value); setArbitro2(e.target.value); setShowSuggest2(true) }}
                      onBlur={() => { setTimeout(() => setShowSuggest2(false), 150); guardarArbitroLocal(arbitroInput2) }}
                      onFocus={() => setShowSuggest2(true)}
                      placeholder="Nombre árbitro..."
                      style={{ ...inpL, width: '120px', display: 'inline', borderBottom: '1px solid #000', marginLeft: '3px' }}/>
                    {showSuggest2 && suggestFiltro2.length > 0 && (
                      <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 500, background: '#fff', border: '1px solid #dadce0', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,.15)', minWidth: '180px', maxHeight: '120px', overflowY: 'auto' }}>
                        {suggestFiltro2.map((a, i) => (
                          <div key={i} onClick={() => { setArbitroInput2(a); setArbitro2(a); setShowSuggest2(false) }}
                            style={{ padding: '5px 10px', cursor: 'pointer', fontSize: '9px', borderBottom: '1px solid #f1f3f4' }}
                            onMouseEnter={e => e.currentTarget.style.background = '#f8f9fa'}
                            onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
                            {a}
                  ...
                  <td rowSpan={2} style={{ ...cell, textAlign:'center', background:'#f1f3f4', width:'20%' }}>
                    ...
                    {hubopenales && penalesGanador && (
                      <div style={{ fontSize: '8px', color: '#1a73e8', fontWeight: '700' }}>
                        Penales: {penalesLocal} — {penalesVisitante}<br/>
                        🏆 {penalesGanador === 'local' ? partido.home?.name : partido.away?.name}
                      </div>
                    )}
