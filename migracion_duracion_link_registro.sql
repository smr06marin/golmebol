-- ============================================================
-- El link de registro de jugadores (el que se comparte desde "🔗 Copiar
-- link de registro" / "Compartir link") vencía siempre a las 24 horas,
-- fijo. Ahora el organizador elige, ANTES de copiarlo, cuánto quiere que
-- dure: 1, 2 o 3 días — se guarda en la nueva columna
-- teams.registro_token_horas (24, 48 o 72).
--
-- Cómo ejecutar: Supabase → SQL Editor → pegar todo → RUN.
-- Es seguro ejecutarlo más de una vez.
-- ============================================================

alter table teams add column if not exists registro_token_horas integer default 24;

-- registrar_equipo: se reescribe completa (igual que en cada migración
-- anterior) solo para que el chequeo de "link vencido" use
-- registro_token_horas en vez de las 24 horas fijas de antes (si un
-- equipo no tiene nada guardado ahí, se asume 24h como antes). El resto
-- de la función queda EXACTAMENTE igual que en
-- migracion_sancion_por_torneo.sql.
